...
def assign_gap_class(classifier, X, Y, threshold=0.1):
    proba = classifier.predict_proba(X)
    confidence = np.max(proba, axis=1)

    # Points with low confidence are treated as gap class (label 2)
...
def augment_gap_class(X, Y, target_class=2):
    # Count how many we have
    n_gap = np.sum(Y == target_class)

    desired_total = 3 * n_gap
